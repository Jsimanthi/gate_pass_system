...
class ReportViewSet(viewsets.ViewSet):
    ...
    def daily_visitor_summary(self, request):
        ...
        if not request.query_params.get('start_date') and not request.query_params.get('end_date'):
            today = timezone.now().date()
            gate_passes = gate_passes.filter(entry_time__date=today)
    ...
    def monthly_visitor_summary_export(self, request):
        ...
        data = gate_passes.values('person_name', 'person_nid', 'person_phone', 'entry_time', 'exit_time', 'purpose__name', 'vehicle__vehicle_number')
    ...
    def daily_visitor_summary_export(self, request):
        ...
        if export_format == 'csv':
            response = HttpResponse(content_type='text/csv')
            response['Content-Disposition'] = 'attachment; filename="daily_visitor_summary.csv"'
            writer = csv.writer(response)
            writer.writerow(['Person Name', 'NID', 'Phone', 'Entry Time', 'Exit Time', 'Purpose', 'Vehicle Number'])
            for row in data:
                writer.writerow(row.values())
            ...
            response = HttpResponse(content_type='application/pdf')
            response['Content-Disposition'] = 'attachment; filename="daily_visitor_summary.pdf"'
    ...
    def security_incident_report_export(self, request):
        ...
        data = incidents.values('gate_pass__person_name', 'security_personnel__username', 'timestamp', 'reason')
    ...
    def monthly_visitor_summary(self, request):
        ...
        try:
            ...
            # No year/month filtering if params are invalid
            pass
